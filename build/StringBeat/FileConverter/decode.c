...
//ideal framegap: startup+active+4f recovery modifier
enum SBSpeed
{
    brief = 4,    //s:3-6f, r:6-9f
    medium = 8,   //s:7-10, r:10-13f
    lengthy = 12, //s:11-14f r:14-17f
    extra = 16    //s:15-18f r:18-21f
};
//Hit heights
enum SBHitHeight
{
...
#define SPECIAL_STALENESS_CAP (4)
    ...
    while (!feof(input))
    {
        ...
        for (int i = 0; i < num_moves; i++)
        {
            ...
            if (FRAME_GAP(current, next) - movelist[i].recovery + 1 >= minimum_startup || next.type == -2)
            {
                if (active_map.command_indices[active_map.length] == -1)
                {
                    ...
                    {
                        //preference based on previous move
                        preference_coeff = abs(FRAME_GAP(previous, current) - movelist[i].startup - movelist[active_map.command_indices[active_map.length - 1]].recovery - 4);
                    }
                    else
                    {
                        //no previous move
                        preference_coeff = abs(FRAME_GAP(current, next) / 2 - movelist[i].recovery);
                    }
                }
                else
                {
                    ...
                    {
                        //no previous move
                        if (abs(FRAME_GAP(current, next) / 2 - movelist[i].recovery) < preference_coeff)
                        {
                            active_map.command_indices[active_map.length] = i;
                            preference_coeff = abs(FRAME_GAP(current, next) / 2 - movelist[i].recovery);
                        }
                    }
                    else if (abs(FRAME_GAP(previous, current) - movelist[i].startup - movelist[active_map.command_indices[active_map.length - 1]].recovery - 4) + staleness[i] / 2 < preference_coeff)
                    {
