...
void DrawGrid(int slices, float spacing)
{
    ...
    if (rlCheckBufferLimit(slices * 4))
        rlglDraw();
...
void ExportMesh(Mesh mesh, const char *fileName)
{
    ...
    if (success)
        TraceLog(LOG_INFO, "Mesh exported successfully: %s", fileName);
    else
        TraceLog(LOG_WARNING, "Mesh could not be exported.");
}
...
Material *LoadMaterials(const char *fileName, int *materialCount)
{
    ...
    for (int i = 0; i < count; i++)
        materials[i].shader = GetShaderDefault();
...
void UnloadMaterial(Material material)
{
    ...
    if (material.shader.id != GetShaderDefault().id)
        UnloadShader(material.shader);

    // Unload loaded texture maps (avoid unloading default texture, managed by raylib)
    for (int i = 0; i < MAX_MATERIAL_MAPS; i++)
    {
        if (material.maps[i].texture.id != GetTextureDefault().id)
    ...
    *animCount = iqm.num_anims;
    ...
    for (int a = 0; a < iqm.num_anims; a++)
    {
        ...
        for (int j = 0; j < anim[a].num_frames; j++)
            animations[a].framePoses[j] = RL_MALLOC(iqm.num_poses * sizeof(Transform));
        ...
        for (int x = 0; x < mapWidth; ++x)
        {
                ...
                (cubicmapPixels[z * cubicmap.width + x].b == 255))
            {
                ...
                if (((z < cubicmap.height - 1) &&
                     ...
                     (cubicmapPixels[(z + 1) * cubicmap.width + x].b == 0)) ||
                    (z == cubicmap.height - 1))
                {
                ...
                if (((x < cubicmap.width - 1) &&
                     ...
                     (cubicmapPixels[z * cubicmap.width + (x + 1)].b == 0)) ||
                    (x == cubicmap.width - 1))
                {
...
void MeshTangents(Mesh *mesh)
{
    if (mesh->tangents == NULL)
        ...
        TraceLog(LOG_WARNING, "Mesh tangents already exist");
...
bool CheckCollisionSpheres(Vector3 centerA, float radiusA, Vector3 centerB, float radiusB)
{
    ...
    */
    ...
    if (Vector3DotProduct(Vector3Subtract(centerB, centerA), Vector3Subtract(centerB, centerA)) <= (radiusA + radiusB) * (radiusA + radiusB))
        collision = true;
...
bool CheckCollisionRaySphereEx(Ray ray, Vector3 center, float radius, Vector3 *collisionPoint)
{
    ...
    if (distance < radius)
        collisionDistance = vector + sqrtf(d);
    else
        collisionDistance = vector - sqrtf(d);
...
static Model LoadOBJ(const char *fileName)
{
    ...
    if (data != NULL)
    {
        ...
        if (ret != TINYOBJ_SUCCESS)
            TraceLog(LOG_WARNING, "[%s] Model data could not be loaded", fileName);
        else
            TraceLog(LOG_INFO, "[%s] Model data loaded successfully: %i meshes / %i materials", fileName, meshCount, materialCount);
...
static Texture LoadTextureFromCgltfImage(cgltf_image *image, const char *texPath, Color tint)
{
    ...
    if (image->uri)
    {
            ...
            (image->uri[4] == ':'))
        {
            ...
            if (image->uri[i] == 0)
                TraceLog(LOG_WARNING, "CGLTF Image: Invalid data URI");
            else
            {
                int size;
...
#define LOAD_ACCESSOR(type, nbcomp, acc, dst)                                                                                      \
    ...
    if (result == cgltf_result_success)
    {
        ...
        TraceLog(LOG_WARNING, "[%s] glTF data could not be loaded", fileName);
